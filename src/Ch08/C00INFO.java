...
public class C00INFO {
	public static void main(String[] args) {
		...
		// 기능(Funtion) : 객체가 수행할 수 있는 Action..-> (멤버)함수를 통해 구현
		
		//3 객체 지향
		// 현실 객체의 정보(속성/기능)중에 필요한 일부만 추출(추상화) 하여 프로그램에 적재하기위해 고안된 문법체계

		//4 클래스
		// 동일한 종류의 객체에 필요한 메모리공간을 제공하기 위한 선언된 자료형
		// 클래스는 선언하기 전에는 기본적으로 메모리 공간을 차지하지 않는다.77
		// 클래스 자료형에 의해 객체가 정의되는 순간 클래스에서 선언한 속성과 기능대로 공간이 형성되어진다.
		
		//5 자바의 메모리 영역
		//1) 스택영역 -> {} 내에서 생성되는 공간 ex(int,double...)
